...
class Metric:
    def __init__(self, name, **kwargs):
        ...
        self.out_dir = args.score_dir if hasattr(args, "score_dir") else "./"
        self.individual_out_dir = os.path.join(self.out_dir, self.name)
        self.store_individual_scores = (
            args.store_individual_scores if hasattr(args, "store_individual_scores") else False
        )
        # file_name is only used when using an LLM evaluation method
        self.file_name = kwargs.get("file_name", f"{name}_scores")
