...
# https://github.com/McGill-NLP/instruct-qa/blob/main/instruct_qa/evaluation/metrics.py
###
####################################
...
from xc_cache.data.utils.templates import HistoryTemplate, PromptTemplate
from xc_cache.metrics import Metric
...
class Meteor(Metric):
    ...
    def __call__(self, predictions, references, questions=None, ids=None):
        ...
        for i in range(len(predictions)):
            scores.append(
                self._metric.compute(predictions=[predictions[i]], references=[references[i]])[
                    "meteor"
                ]
            )
...
class BERTScore(Metric):
    ...
    def __call__(self, predictions, references, questions=None, ids=None):
        scores = self._metric.compute(predictions=predictions, references=references, lang="en")
        if self.store_individual_scores:
            individual_scores = []
            for i in range(len(predictions)):
...
class Rouge(Metric):
    ...
    def __call__(self, predictions, references, questions=None, ids=None):
        ...
        if self.store_individual_scores:
            individual_scores = []
            for i in range(len(predictions)):
                individual_scores.append({name: scores[name][i] for name in self._rouge_types})
            self.save_individual_scores(ids, individual_scores)
