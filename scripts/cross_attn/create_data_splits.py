...
def create_hotpotqa_splits(cache_dir: str) -> datasets.Dataset:
    ...
    train_val_split = hotpotqa_data["train"].train_test_split(
...
class TopiOCQASplitFilter:
    ...
    def __call__(self, example: Dict[str, Any]) -> bool:
        return self.train and example["Conversation_no"] <= self.conversation_cutoff_idx or not self.train and example["Conversation_no"] > self.conversation_cutoff_idx
...
def create_topiocqa_splits(cache_dir: str, num_proc: int=1) -> datasets.Dataset:
    ...
    train_split = topiocqa_data["train"].filter(
        TopiOCQASplitFilter(conversation_cutoff_idx=conversation_cutoff_id, train=True), num_proc=num_proc)
    
    val_split = topiocqa_data["train"].filter(
        TopiOCQASplitFilter(conversation_cutoff_idx=conversation_cutoff_id, train=False), num_proc=num_proc)
